...
import (
	...
	"github.com/jtieri/habbgo/legacy/game/jobs"
...
func G_HMAP(player player.Player, packet packets.IncomingPacket) {
	proxy := player.Services.RoomService().(*room2.RoomServiceProxy)
...
func GOAWAY(p player.Player, packet packets.IncomingPacket) {
	roomService := p.Services.RoomService().(*room2.RoomServiceProxy)
...
func MOVE(player player.Player, packet packets.IncomingPacket) {
	...
	roomService.MovePlayer(player.State().RoomID, room2.NewPlayerMove(player, xCoord, yCoord))
}

func enterRoom(rs *room2.RoomServiceProxy, r room2.Room, player player.Player) error {
	ctx, cancel := context.WithCancel(player.Ctx)
	roomStartupJobs := []scheduler.Job{jobs.NewPlayerJob(ctx, cancel, r.Details.Id, *player.Services.RoomService().(*room2.RoomServiceProxy))}

	fmt.Println("Before room service proxy call EnterRoom")
	// TODO remove indiretion after refactor away from pointers
	rs.EnterRoom(r.Details.Id, room2.NewPlayerEnter(player, roomStartupJobs))
	...
	if r.Details.Wallpaper > 0 {
		player.Session.Send(messages.FLATPROPERTY, messages.FLATPROPERTY(room2.WallpaperProperty, r.Details.Wallpaper))
	}

	if r.Details.Floor > 0 {
		player.Session.Send(messages.FLATPROPERTY, messages.FLATPROPERTY(room2.FloorProperty, r.Details.Floor))
	}
