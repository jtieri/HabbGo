...
func GETAVAILABLESETS(p player.Player, packet packets.IncomingPacket) {
	p.Session.Send(messages2.AVAILABLESETS, messages2.AVAILABLESETS())
}

func GDATE(p player.Player, packet packets.IncomingPacket) {
	p.Session.Send(messages2.DATE, messages2.DATE(date.CurrentDate()))
}

func APPROVENAME(p player.Player, packet packets.IncomingPacket) {
	name := text.Filter(packet.ReadString())
	p.Session.Send(messages2.APPROVENAMEREPLY, messages2.APPROVENAMEREPLY(checkName(p, name)))
}

func APPROVE_PASSWORD(p player.Player, packet packets.IncomingPacket) {
	username := packet.ReadString()
	password := packet.ReadString()
	p.Session.Send(messages2.PASSWORD_APPROVED, messages2.PASSWORD_APPROVED(checkPassword(p, username, password)))
}
...
func APPROVEEMAIL(p player.Player, packet packets.IncomingPacket) {
	...
	if _, err := mail.ParseAddress(email); err != nil {
		p.Session.Send(messages2.EMAIL_REJECTED, messages2.EMAIL_REJECTED())
	} else {
		p.Session.Send(messages2.EMAIL_APPROVED, messages2.EMAIL_APPROVED())
	}
}
