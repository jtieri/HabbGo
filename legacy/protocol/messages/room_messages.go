...
func OBJECTS(items []item2.Item) packets.OutgoingPacket {
...
func ACTIVE_OBJECTS(items []item2.Item) packets.OutgoingPacket {
...
func SerializeItems(items []item2.Item, packet packets.OutgoingPacket) packets.OutgoingPacket {
	for _, i := range items {
		itemState := i.State

		switch {
		// handle public room items
		case i.Definition.ContainsBehavior(item2.PublicSpaceObject):
			...
			if i.Definition.ContainsBehavior(item2.ExtraParameter) {
				packet.Write(" 2")
			}
		...
		// handle wall item
		case i.Definition.ContainsBehavior(item2.WallItem):
			...
			if i.CustomData != "" {
				if i.Definition.ContainsBehavior(item2.PostIt) {
		...
		default:
			...
			if i.Definition.ContainsBehavior(item2.Roller) {
				packet.WriteInt(2)
			} else {
				packet.WriteInt(0)
			}
			...
			if i.Definition.ContainsBehavior(item2.Present) {
				presentData := strings.Split(i.CustomData, item2.PresentDelimiter)
