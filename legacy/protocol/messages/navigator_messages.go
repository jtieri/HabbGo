...
func NAVNODEINFO(
	...
	*/
	...
	for _, subcat := range subcats {
		...
		r := player.Services.RoomService().(*room2.RoomServiceProxy).Rooms()
		p.WriteInt(subcat.ID)
		p.WriteInt(0)
		p.WriteString(subcat.Name)
		p.WriteInt(room2.CurrentVisitors(subcat, r)) // writeInt currentVisitors
		p.WriteInt(room2.MaxVisitors(subcat, r))     // writeInt maxVisitors
		p.WriteInt(parentCat.ID)
	}
