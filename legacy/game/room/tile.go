...
type Tile struct {
	...
	items   collections.Cache[int, item2.Item]
...
func NewTile() Tile {
	return Tile{
		...
		items:    collections.NewCache(make(map[int]item2.Item)),
		TopItem:  item2.Item{},
	}
}
...
func (t *Tile) AddItem(item item2.Item) {
...
func (t *Tile) WalkingHeight() float64 {
	...
	if t.TopItem.Definition.ContainsBehavior(item2.CanSitOnTop) || t.TopItem.Definition.ContainsBehavior(item2.CanLayOnTop) {
