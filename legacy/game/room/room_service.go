...
import (
	...
	"github.com/jtieri/habbgo/legacy/game/scheduler"
	"github.com/jtieri/habbgo/legacy/game/service/query"
...
func newServiceChannel() *ServiceChannels {
	return &ServiceChannels{
		RoomAddChan:          make(chan *query.Request[int, Room]),
		RoomDeleteChan:       make(chan *query.Request[int, Room]),
		RoomByPlayerID:       make(chan *query.Request[int, []Room]),
		RoomByRoomID:         make(chan *query.Request[int, Room]),
		RoomsChan:            make(chan *query.Request[int, []Room]),
		RoomCachedChan:       make(chan *query.Request[int, bool]),
		RoomCountChan:        make(chan *query.Request[int, int]),
		RoomQueriedChan:      make(chan *query.Request[int, []Room]),
		RoomPlayersChan:      make(chan *query.Request[int, []player.Player]),
		RoomAddPlayerChan:    make(chan *query.Request[int, player.Player]),
		RoomRemovePlayerChan: make(chan *query.Request[int, int]),
		RoomPlayerCountChan:  make(chan *query.Request[int, int]),
		RoomAddItemsChan:     make(chan *query.Request[int, []item.Item]),
		RoomFloorItemsChan:   make(chan *query.Request[int, []item.Item]),
		RoomPublicItemsChan:  make(chan *query.Request[int, []item.Item]),
		RoomPublicRoomChan:   make(chan *query.Request[int, bool]),
		RoomUpdateChan:       make(chan *query.Request[int, Room]),
		RoomIsHCChan:         make(chan *query.Request[int, bool]),
		RoomSendPacketChan:   make(chan *query.Request[int, packets.OutgoingPacket]),
		RoomScheduleJobChan:  make(chan *query.Request[int, scheduler.Job]),
		RoomStopJobChan:      make(chan *query.Request[int, string]),
		RoomPlayerEnterChan:  make(chan *query.Request[int, PlayerEnter]),
		RoomPlayerLeaveChan:  make(chan *query.Request[int, player.Player]),
		RoomPlayerMove:       make(chan *query.Request[int, PlayerMove]),
	}
}
