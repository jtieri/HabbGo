...
type Room struct {
	...
	playerCache collections.Cache[int, player2.Player]
	itemCache   collections.Cache[int, item2.Item]
...
func NewRoom() Room {
	return Room{
		...
		mapping:     Map{},
		Initialized: false,
		playerCache: collections.NewCache(make(map[int]player2.Player)),
		itemCache:   collections.NewCache(make(map[int]item2.Item)),
...
func (r *Room) AddPlayer(instanceID int, player player2.Player) {
...
func (r *Room) floorItems() []item2.Item {
	var items []item2.Item

	for _, i := range r.itemCache.Items() {
		if i.Definition.ContainsBehavior(item2.PublicSpaceObject) || i.Definition.ContainsBehavior(item2.WallItem) {
			continue
		}
...
func (r *Room) publicRoomItems() []item2.Item {
	var items []item2.Item

	for _, i := range r.itemCache.Items() {
		if i.Definition.ContainsBehavior(item2.Invisible) ||
			i.Definition.ContainsBehavior(item2.PrivateFurniture) ||
			!i.Definition.ContainsBehavior(item2.PublicSpaceObject) {
			continue
		}
...
func NewPlayerEnter(p player2.Player, jobs []scheduler.Job) PlayerEnter {
	return PlayerEnter{
		Player: p,
		Jobs:   jobs,
	}
...
func (r *Room) enterRoom(p player2.Player, jobs ...scheduler.Job) error {
...
func (r *Room) loadRoom(player player2.Player, jobs ...scheduler.Job) {
	...
	itemService := player.Services.ItemService().(*item2.ItemServiceProxy)
	if r.publicRoom() {
...
func (r *Room) leaveRoom(p player2.Player) error {
...
func NewPlayerMove(p player2.Player, xCoord, yCoord int) PlayerMove {
	return PlayerMove{
		Player: p,
		XCoord: xCoord,
		YCoord: yCoord,
