...
// transition.
func ValidStep(p *player.Player, r room2.Room, currentPos, testPos position.Position, finalMove bool) bool {

	if !room2.ValidTile(currentPos, r) || !room2.ValidTile(testPos, r) {
		return false
	}
	...
	if validDiagonalStep {
		if currentPos.X != testPos.X && currentPos.Y != testPos.Y {
			firstValidTile := room2.ValidTile(position.Position{X: testPos.X, Y: currentPos.Y}, r)
			secondValidTile := room2.ValidTile(position.Position{X: currentPos.X, Y: testPos.Y}, r)
	...
	if !currentPos.Equals(r.Model.Door.Position()) {
		...
		return endItem.Definition.ContainsBehavior(item2.CanSitOnTop) || endItem.GateOpen()
	}
...
// skipItem
func skipItem(itemOne, itemTwo item2.Item) bool {
	if itemOne.Definition.ContainsBehavior(item2.Teleporter) ||
...
func NewPath(currentPos, endPos position.Position, player *player.Player, room room2.Room) list.List {
	var path list.List
...
func NewPathReverse(currentPos, endPos position.Position, player *player.Player, room room2.Room) *Node {
