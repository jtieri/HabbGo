...
import (
	...
	"github.com/jtieri/habbgo/legacy/protocol/encoding"
	"github.com/jtieri/habbgo/legacy/protocol/messages"
	packets2 "github.com/jtieri/habbgo/legacy/protocol/packets"
	"go.uber.org/zap"
)
...
func packetFromBytes(packetBytes []byte) (packets2.IncomingPacket, error) {
	...
	for i := 0; i < 2; i++ {
		...
		if err != nil {
			return packets2.IncomingPacket{}, err
		}
		rawHeader[i] = b
	}

	return packets2.NewIncoming(rawHeader, payload), nil
}
...
func (session *Session) handle(p player2.Player, packet packets2.IncomingPacket) {
...
func (session *Session) Send(caller interface{}, packet packets2.OutgoingPacket) {
	packet.Finish()
...
func (session *Session) Queue(caller interface{}, packet packets2.OutgoingPacket) {
	packet.Finish()
	session.buffer.mux.Lock()
	defer session.buffer.mux.Unlock()
...
func (session *Session) Flush(caller interface{}, packet packets2.OutgoingPacket) {
	session.buffer.mux.Lock()
	defer session.buffer.mux.Unlock()
